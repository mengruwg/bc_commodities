...
pca.1980 <- readRDS("data/comm_pca_1980.rds")
...
# pca.comm prep.
pca.1980.comm <- cbind(pca.1980$TIME, pca.1980$commodities)
colnames(pca.1980.comm) <- c("TIME", "Value")
pca.1980.comm <- as.data.frame(pca.1980.comm)

# pca. ind. met. prep
pca.1980.ind.met <- cbind(pca.1980$TIME, pca.1980$ind_metal)
...
colnames(pca.1980.stat) <- c("TIME", "comm", "industr")

saveRDS(pca.1980.stat, "data/comm_pca_1980_transformed.rds")
